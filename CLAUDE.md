...
GitHub milestone **`Alpine 0.1.0`** (`soroushkarahrodi79-oss/snto-alpine`, milestone #1): *"Prototipo reproducible: aislamiento operativo, backlog trazable, QA visual y release honesta sin validación de campo."* As of 2026-08-23: 4 open issues (incl. #27), 7 closed.
...
| #27 | Surface richer real IECA/SIMA municipal indicators in Contexto municipal | **Open — parser extended for more real ficha fields; snapshot rebuilt live; table shows curated subset** |
...
- `src/socioeconomic/alpine_indicators.py` (issues #22, #27) — parses IECA/SIMA's "Andalucía pueblo a pueblo" municipal fact sheets into real population/economy figures, one committed snapshot per municipality (`src/socioeconomic/snapshot/sierra_nevada_municipal_indicators.json`, built by `scripts/build_alpine_municipal_indicators.py`). IECA's own suppression marks (`*`/`-`, small-municipality statistical secrecy) parse to `None` with an explicit caveat, never a fabricated number. Issue #27 extended `_LABEL_FIELD_MAP` to capture more real fields **from the same ficha** (no separate bulk export needed): real-estate transactions (nueva/2ª mano), total establishments, agricultural surface (herbáceos/leñosos ha — plural "surface" needles must not catch the singular "principal cultivo" decoy rows), electricity consumption (total + residential), municipal income/expense per capita, and hostales/pensiones (usually `*`-suppressed in these small municipalities). The `Contexto municipal` table in `tab_alpine.py` surfaces the 24/24-coverage subset (establecimientos, transacc. 2ª mano, consumo eléctrico, ingresos/hab); the rest live in the snapshot.
...
4. Issue #27 (open) picked up the "richer municipal indicators" idea — but sourced from the **same SIMA ficha** the pipeline already scrapes (real-estate tx, establishments, agricultural surface, electricity, income/expense per capita, hostales), not the separate bulk export. Still on the table if wanted: election results per party (present in the ficha but low decision-value / noisy to parse — deliberately skipped) and text fields (principal cultivo, principal CNAE sector — the current parser is numeric-only). File a follow-up issue before pursuing those.
